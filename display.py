...
def visualize_data(data, latitude_user, longitude_user, perimetre_user, query_element):
    ...
    for row in data:
        ...
        distance = int(geopy.distance.geodesic(coords_user, coords_resto).meters)

        # Définir l'icône en fonction du type d'établissement
        if query_element == "Restaurant":
            icon = folium.CustomIcon(r'C:\Users\Mejri Wissem\Documents\DATA_SCIENTEST\PROJET\wis\icone_restaurant.png', icon_size=(50, 50))
        elif query_element == "Hotel":
            icon = folium.CustomIcon(r'C:\Users\Mejri Wissem\Documents\DATA_SCIENTEST\PROJET\wis\icone_hotel.png', icon_size=(50, 50))
        else:
            icon = folium.Icon(color='blue')  # Icône par défaut
        
        if float(perimetre_user) > float(distance):
            if distance >= 1000:
                distance_str = "{:.2f} km".format(distance / 1000)
            else:
                distance_str = "{:.0f} m".format(distance)
            popup_description = (
                                f"<b><u>{name}</u></b><br><br>"
                                f"<b>Distance</b> : {distance_str} <br><br>"
                                f"<b>Telephone</b> : {telephone}<br><br>"
                                )
